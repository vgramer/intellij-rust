...
class RsCoverageTest : RunConfigurationTestBase() {
    override val executor: Executor
        get() = ExecutorRegistry.getInstance().getExecutorById(CoverageExecutor.EXECUTOR_ID)!!
    ...
    fun `test tests`() = doTest(runTests = true) {
        toml("Cargo.toml", """
            [package]
            name = "hello"
            version = "0.1.0"
    ...
    private fun doTest(runTests: Boolean = false, builder: FileTreeBuilder.() -> Unit) {
        ...
        execute(configuration, CoverageRunnerData())
