        ...
        while (macros.isNotEmpty()) {
            ...
            for (element in macro.elementsForHighlighting) {
                for (ann in annotators) {
                    ...
                    ann.annotate(element, holder)
                }
                if (element is RsMacroCall) {
                    macros += element.prepare() ?: continue
                }
            }
            mapAndCollectAnnotations(macro, holder)
            holder.clear()
        }
    }

    private fun mapAndCollectAnnotations(macro: PreparedMacroCall, annotations: AnnotationHolderImpl) {
...
private fun HighlightInfo.copyWithRange(newRange: TextRange): HighlightInfo {
    ...
    if (forcedTextAttributesKey != null) b.textAttributes(forcedTextAttributesKey)
    else if (forcedTextAttributes != null) b.textAttributes(forcedTextAttributes)

    if (description != null) b.description(description)
    if (toolTip != null) b.escapedToolTip(toolTip)
    if (isAfterEndOfLine) b.endOfLine()
