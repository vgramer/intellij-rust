...
class RsSdkType : SdkType(RUST_SDK_ID_NAME) {
    ...
    override fun suggestHomePath(): String? {
        ...
        val sdks = detectRustSdks(null, existingSdks)
    ...
    override fun getHomeChooserDescriptor(): FileChooserDescriptor =
        object : FileChooserDescriptor(true, true, false, false, false, false) {
            ...
            override fun isFileVisible(file: VirtualFile, showHiddenFiles: Boolean): Boolean =
                if (!file.isDirectory && SystemInfo.isWindows) {
                    file.path.endsWith(".exe") && super.isFileVisible(file, showHiddenFiles)
                } else {
    ...
    override fun getVersionString(sdkPath: String?): String? {
        val flavor = RsSdkFlavor.getFlavor(sdkPath)
        return flavor?.getVersionString(sdkPath)
    }
